...
class SetCriterion(nn.Module):
    ...
    def loss_labels_(self, outputs, targets, indices, num_boxes, log=True):
    ...
    def loss_labels(self, outputs, targets, indices, num_boxes, log=True):
        """Classification loss (Polarity Loss)
        targets dicts must contain the key "labels" containing a tensor of dim [nb_target_boxes]
        """
    ...
    #>>>> HOI Losses 4 : pair interaction
    def loss_pair_interactions(self, outputs, targets, hoi_indices, num_interactions, log=True):
        assert 'pred_interactiveness' in outputs
        src_logits = outputs['pred_interactiveness']
        idx = self._get_src_permutation_idx(hoi_indices)
        
        target_classes_o = torch.cat([t['pair_interactions'][J] for t, (_, J) in zip(targets, hoi_indices)])
        pad_tgt = -1 # src_logits.shape[2]-1
        target_classes = torch.full(src_logits.shape[:2], pad_tgt, dtype=torch.int64, device=src_logits.device)
        target_classes[idx] = target_classes_o
        # target_classes.shape torch.Size([2, 16])
        loss_interaction = F.cross_entropy(src_logits.transpose(1, 2), target_classes, ignore_index=-1)
        # ****************************************************************************************************************************
        # ****************************************************************************************************************************
    ...
    def get_HOI_loss(self, loss, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if self.dataset_file == 'hico-det': 
            loss_map['pair_targets'] = self.loss_pair_targets
            loss_map['pair_interactions'] = self.loss_pair_interactions
        assert loss in loss_map, f'do you really want to compute {loss} loss?'
        return loss_map[loss](outputs, targets, indices, num_boxes, **kwargs)
    # *****************************************************************************
