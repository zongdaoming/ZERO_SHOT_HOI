...
class HOTR(nn.Module):
    def __init__(self, 
        ...
        # TODO
        self.object_embed =  nn.Linear(hidden_dim, object_d_model)
    ...
    def forward(self, samples: NestedTensor):
        ...
        inst_repr = F.normalize(instance_hs[-1], p=2, dim=2) # instance representations inst_repr.shape torch.Size([2, 100, 256])

        # prediction heads for object detection
        # self.detr.class_embed.shape torch.Size([92, 300])
        self.obj_classifier = self.detr.class_embed # obj_class_logits.shape  torch.Size([92, 300])
        # outputs_class = self.detr.class_embed(instance_hs) # outputs_class.shape torch.Size([6, 2, 100, 92])
        outputs_class = torch.stack([torch.matmul(F.normalize(self.object_embed(inst_repr_elem), p=2, dim=2), self.obj_classifier.transpose(0, 1)) for inst_repr_elem in instance_hs])
