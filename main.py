...
def main(args):
    ...
    if args.dataset_file == 'vcoco':
        ...
        # Save HICO-DET stastics
        args.valid_obj_ids = dataset_train.get_valid_obj_ids()
        args.valid_verb_id_to_verb_names = dataset_train.get_valid_verb_id_to_verb_names()
        args.valid_obj_ids_to_names = dataset_train.get_valid_obj_ids_to_names()

    utils.print_args(args) # print_args(args)
    ...
    if args.frozen_weights is not None:
        if args.backbone == 'resnet50':                
            if args.pretrained_resnet50.startswith('https'):
                checkpoint = torch.hub.load_state_dict_from_url(
                    args.pretrained_resnet50, map_location='cpu', check_hash=True)
            else:
                checkpoint = torch.load(args.pretrained_resnet50, map_location='cpu')
            model_without_ddp.detr.load_state_dict(checkpoint['model'],strict=False)
        elif args.backbone == 'resnet101':                
            if args.pretrained_resnet101.startswith('https'):
                checkpoint = torch.hub.load_state_dict_from_url(
                    args.pretrained_resnet101, map_location='cpu', check_hash=True)
            else:
                checkpoint = torch.load(args.pretrained_resnet101, map_location='cpu')
            model_without_ddp.detr.load_state_dict(checkpoint['model'],strict=False)
    ...
    if args.eval:
        ...
        if args.HOIDet:
            if args.dataset_file == 'vcoco':
                ...
                print(f'| mAP (seen)\t: {test_stats["mAP seen"]:.2f}')
                print(f'| mAP (unseen)\t: {test_stats["mAP unseen"]:.2f}')
    ...
    best_mAP, best_rare, best_seen, best_unseen, best_non_rare = 0, 0, 0, 0, 0
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args.clip_max_norm, dataset_file=args.dataset_file, log=False)
        lr_scheduler.step()
        # Validation
        if args.validate:
            print('-'*100)
            if args.dataset_file == 'vcoco':
                total_res = hoi_evaluator(args, model, criterion, postprocessors, data_loader_val, device)
                if utils.get_rank() == 0:
                    sc1, sc2 = hoi_accumulator(args, total_res, False, True)
                    if sc1 > scenario1:
                        scenario1 = sc1
                        scenario2 = sc2
                ...
                if utils.get_rank() == 0:
                    if test_stats['mAP'] > best_mAP:
                        ...
                        best_seen = test_stats['mAP seen']
                        best_unseen = test_stats['mAP unseen']
                        save_ckpt(args, model_without_ddp, optimizer, lr_scheduler, epoch, filename='best')
                    ...
                    print(f'| mAP (seen)\t: {test_stats["mAP seen"]:.2f} ({best_seen:.2f})')
                    print(f'| mAP (unseen)\t: {test_stats["mAP unseen"]:.2f} ({best_unseen:.2f})')
                    logger.info(f'| mAP: {test_stats["mAP"]}')
                    # if args.wandb and utils.get_rank() == 0:
                    #     wandb.log({
                    #         'mAP': test_stats['mAP']
                    #     })
    ...
    if args.dataset_file == 'vcoco':
        ...
        print(f'| mAP (seen)\t: {best_seen:.2f}')
        print(f'| mAP (unseen)\t: {best_unseen:.2f}')
